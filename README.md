...
> **Codec** = **co**der + **dec**oder：把对方的话解码成真实意图，把你的真心话编码成得体表达，并通过用户反馈持续提升个体针对性。

Codec98 是一个面向中文高语境沟通场景的双向语言转换工具。它将「话里有话」的表达翻译为可理解、可行动的信息，覆盖职场、恋爱、甲方、互联网四个领域。服务端为零依赖 Node.js 实现，支持阿里云百炼（DashScope）与 Anthropic 双 LLM 后端，并内置本地规则引擎作为离线降级层。
...
- **正向解码（DECODE）**：输入对方原话，输出 1–2 个按可能性排序的潜台词解读，每个解读附带可直接发送的回复草稿。系统会先判断输入是否为字面含义的事务性表述，避免过度解读；当多个解读难分高下时，额外给出低成本的试探建议
- **反向编码（ENCODE）**：输入情绪化的真心话，输出立场不变但表达得体的改写。核心原则是保留信息内核（拒绝仍是拒绝、反对仍是反对），只替换包装

### 语气档位

同一解读提供三档回复草稿：🛡️ 稳妥 → 不卑不亢 → 🔥 勇。拖动滑块即时切换，从"圆滑留余地"到"直接对质"。

![解码报告](docs/screenshots/03-decode-report.png)

### 多领域架构

四个领域完全由配置驱动，切换领域时标题栏、输入提示、评估仪表、人物类型体系、鉴定问卷全部随之变化：
...
![互联网领域](docs/screenshots/05-internet-domain.png)

### 人物档案与个性化

通过 5 题问卷为具体的人建立档案：类型判定、五维雷达图、口头禅记录、研究等级（Lv.1–9，随语料量增长）。档案按领域隔离存储。

![人物档案卡](docs/screenshots/04-boss-card.png)

### 反馈闭环

每次解码后可标记「翻译准」或提交人工纠正。纠正内容写入该人物的专属语料库，在后续解码的 prompt 中以**最高优先级**注入——用户确认的事实优先于任何通用规律。并排对比模式可同屏查看通用解码与注入个人语料后的专属解码差异。

### 设计约束

产品层面遵守三条硬性规则，写入所有领域的系统提示词：
...
启动日志会标明当前推理后端：`LLM在线（dashscope）` 或本地引擎。
...
ANTHROPIC_API_KEY=          # 备选后端；两者均配置时优先使用百炼
PORT=3210
```
...
lib/llm.js                 LLM 调用层：DashScope / Anthropic / 本地规则引擎
lib/store.js               JSON 持久化与领域配置加载
data/domains.json          领域定义（文案/仪表/类型/问卷/雷达维度/系统提示词）
...
data/bosses.json           人物档案存储
data/corpus.json           人物专属语料与纠错记录
...
LLM 输出强制为 JSON（DashScope 走 `response_format: json_object`；Anthropic 走文本抽取 + 解析），字段包括解读列表、回复草稿、双仪表数值与试探建议。

### 三级推理降级

```
DashScope（百炼） → Anthropic → 本地规则引擎
```

任一后端调用失败（未配置、网络异常、API 错误）自动降级到下一级，服务本身永不因外部依赖不可用而失效。本地规则引擎由三部分组成：领域关键词正则匹配、语料库字符重叠度模糊匹配（阈值 0.45）、通用兜底策略。界面右下角如实标注每次结果的来源（`⚡ LLM 实时解码` / `⚙ 本地引擎`）。

### 其他实现要点

- **前端无构建**：98.css + 原生 JS + 原生 fetch，`public/` 目录直接静态服务
- **音效零素材**：8-bit 提示音由 WebAudio API 方波振荡器实时合成
- **XSS 防护**：所有用户输入与 LLM 输出经统一转义函数处理后再插入 DOM
- **数据持久化**：JSON 文件存储，适配单机部署场景，无外部数据库依赖
...
`compare: true` 时返回 `{generic, custom}` 双结果，用于并排对比。
