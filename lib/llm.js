...
const DEMO_CASES = {
  ...
  employee: [
    {
      match: t => /知道了|明白了|收到/.test(t),
      generic: {
        literal: false,
        interpretations: [
          { label: '最可能', 潜台词: '"知道了"回得越快、追问越少，越可能没真听懂或没认同——真理解的人通常会确认一两个细节。此刻TA可能只是想结束这段对话', 回复草稿: { safe: '好，那你先说说你打算怎么入手？我听听思路。', neutral: '行。为了确保咱俩理解一致，你用一分钟复述一下这件事的重点和你的第一步，我确认下没偏差。', brave: '"知道了"这三个字我听过太多次了（笑）。来，说说你理解的是什么、打算怎么做、什么时候给我看第一版——说得出来我就放心了。' } },
          { label: '也有可能', 潜台词: '真的知道了，这事对TA来说确实简单，或者之前已经沟通过类似任务', 回复草稿: { safe: '好，有不清楚的随时来问我。', neutral: '好。周三你先发我一版中间进度，有问题咱们早发现早调。', brave: '行，那这事我就不盯了，周五直接看结果。有坑提前说，别憋到最后一天。' } }
        ],
        探口风建议: '最便宜的验证：让TA复述任务重点或说出第一步。答得出来=真知道；支支吾吾或原话重复你的话=需要重新讲一遍，并约中间检查点。',
        指数A: 60, 指数B: 10
      }
    },
    {
      match: t => /没问题|包在我|保证完成/.test(t),
      generic: {
        literal: false,
        interpretations: [
          { label: '最可能', 潜台词: '答应得越痛快，越可能没评估过工作量。这是社交性承诺（让你现在满意），不是执行性承诺（让事情按时完成）', 回复草稿: { safe: '好！那你估一下大概需要几天？我心里有个数。', neutral: '痛快！那咱们把它落实一下：你评估下工作量，明天上午给我一个时间点和里程碑，就按那个执行。', brave: '先别急着保证（笑）。你先花半天评估一下，明天告诉我：需要几天、卡点可能在哪、要我什么支持。评估完的承诺我才当真。' } }
        ],
        探口风建议: '',
        指数A: 55, 指数B: 5
      }
    ...
    {
      match: t => /一切顺利|按计划|没什么问题/.test(t),
      generic: {
        literal: false,
        interpretations: [
          { label: '最可能', 潜台词: '连续的"一切顺利"值得警惕——真实推进的项目一定有磕绊。可能是问题被藏着，打算自己扛到扛不住为止', 回复草稿: { safe: '挺好。那你现在最担心的一个风险是什么？咱们提前想想。', neutral: '好。换个问法：如果这个项目最后延期了，最可能是因为什么？——我不是怀疑进度，是想把雷提前排了。', brave: '连续三周"一切顺利"，我反而睡不着（笑）。把你最担心的三件事列给我，哪怕很小——报忧不扣分，瞒住了才扣。' } },
          { label: '也有可能', 潜台词: '确实顺利，任务在TA能力舒适区内', 回复草稿: { safe: '好，保持节奏。', neutral: '好。既然有余力，你看看有没有想顺手优化的地方，做完这个咱们聊聊下一步给你加点有挑战的。', brave: '顺利就好。那我给你加点料：这块做完你来牵头下一个模块，敢不敢接？' } }
        ],
        探口风建议: '问"有没有问题"永远得到"没有"。改问"最大的风险是什么"或"如果延期会因为什么"——具体的问题才能钓出具体的答案。',
        指数A: 55, 指数B: 15
      }
    },
    {
      match: t => /好意思|这点产出|混日子|白养|滚蛋|干不了就/.test(t),
      reverse: {
        literal: false,
        interpretations: [
          { label: '润色版', 潜台词: '把"人格攻击"降级为"结果差距+改进路径"。骂完的员工只会学会隐瞒，不会学会干活——批评行为、给出标准、留出路径，话才能进得去', 回复草稿: { safe: '这个月的产出和我的预期有差距，我想听听你这边遇到了什么困难，咱们一起看看怎么调整。', neutral: '直说：这个月的结果我不满意，具体是A和B两项没达标。我需要你下周给我一个改进计划——是方法问题、投入问题还是我这边的支持不够，咱们摊开对一次。', brave: '我把话说透：目前的产出水平持续下去是过不了考核的，这不是吓唬你。但我今天找你不是为了批评——两周时间，你拿一个看得见的改变给我，需要什么支持现在提。丑话和支持我都放这儿了。' } }
        ],
        指数A: 10, 指数B: 15
      }
    }
  ...
  internet: [
    {
      match: t => /对齐|颗粒度|链路|闭环|抓手|赋能|组合拳/.test(t),
      generic: {
        literal: false,
        interpretations: [
          { label: '最可能', 潜台词: '黑话浓度超标通常意味着：说话的人自己也没想清楚具体要做什么，需要用词汇的密度掩盖思路的稀疏。真正的风险是——落地的模糊任务最后都会落到你头上', 回复草稿: { safe: '好的！我先整理一份会议纪要，把咱们要对齐的具体事项列出来，您看有没有遗漏？', neutral: '收到。为了高效推进，我把这件事拆成三个具体问题：1）范围是什么 2）谁负责哪块 3）什么时间交付。咱们逐条确认一下？', brave: '咱们说人话版本（笑）：具体要做什么、谁做、什么时候要？这三个问题定了，链路自然就通了。' } },
          { label: '也有可能', 潜台词: '他在练习向上汇报的语言体系，这段话本来是说给他老板听的，你只是听众', 回复草稿: { safe: '明白，那我先按现有计划推进，有需要对齐的地方随时找我。', neutral: '好的。落到我这边的部分，我理解是X和Y两块，我先推进这两块，有偏差您纠正。', brave: '行，汇报口径您定。我只确认一件事：我这边的交付物和时间点不变，对吧？' } }
        ],
        探口风建议: '问一句"具体第一步做什么"是最好的试金石：能立刻答上来=真有规划；开始新一轮黑话=他也没想好，尽快把你的职责范围书面化。',
        指数A: 95, 指数B: 40
      }
    },
    {
      match: t => /很简单|加个按钮|小需求|顺手/.test(t),
      generic: {
        literal: false,
        interpretations: [
          { label: '最可能', 潜台词: '"很简单"是需求方对你工作量的单方面估价。按钮背后的接口、状态处理、兼容性、埋点，在对方的想象里都不存在', 回复草稿: { safe: '收到！我今天评估一下涉及的模块，明天上午给您准确的排期哈。', neutral: '好的。这个改动我需要评估一下影响面——看起来是个按钮，背后可能涉及接口和数据。今天下班前给您评估结论和排期。', brave: '"简单"不"简单"我评估完才知道（笑）。老规矩：需求文档发我，我给排期——口头需求不进排期，这是保护咱们双方。' } }
        ],
        探口风建议: '',
        指数A: 55, 指数B: 75
      }
    ...
    {
      match: t => /不要求.*加班|节点是死的|多冲一冲|狼性/.test(t),
      generic: {
        literal: false,
        interpretations: [
          { label: '最可能', 潜台词: '翻译：要求加班，但不想承担"要求加班"的表述责任。"节点是死的"言外之意是你的下班时间是活的', 回复草稿: { safe: '明白节点的重要性，我先把手头任务的优先级排一下，保证关键交付。', neutral: '收到。按现有人力和正常工时，节点前能保质交付A和B，C有风险。您看是调整范围、加资源，还是移节点？我需要您拍个方向。', brave: '节点是死的，人是活的，但人的产出上限也是死的（笑）。我把三个选项摆桌面上：砍范围、加人、延期——您选一个，别默认我的睡眠是项目缓冲区。' } },
          { label: '也有可能', 潜台词: '他自己也被上面压了节点，这句话是压力的向下传导，他其实在等你给他一个能向上交代的方案', 回复草稿: { safe: '理解，大家压力都大。我把我这块的进度和风险整理一份给您，您向上同步也方便。', neutral: '我猜您那边压力也不小。这样：我出一份带风险标注的进度表，哪里可能延期、原因是什么，您拿去向上对齐，咱们把压力变成信息。', brave: '咱们结盟吧：我保证透明进度不藏雷，您负责向上争取合理的节点。压力传导到我这里就停了——再往下传就要出 bug 了（笑）。' } }
        ],
        探口风建议: '观察他自己走不走：说完"节点是死的"自己八点下班=纯管理话术；自己也在陪跑=压力真实来自上层，值得跟他结盟而不是对抗。',
        指数A: 70, 指数B: 95
      }
    },
    {
      match: t => /做不了|排期.*爆|不想加班|需求.*垃圾|拒绝/.test(t),
      reverse: {
        literal: false,
        interpretations: [
          { label: '润色版', 潜台词: '把"拒绝"翻译成"资源与优先级问题"——在互联网语境里，谈排期永远比谈意愿安全：意愿可以被批评，排期只能被调配', 回复草稿: { safe: '这个需求我看了，想做好的话需要X天。我手头Y项目也在节点上，您帮我看看优先级怎么排？', neutral: '同步一下我的排期现状：当前队列里有A、B两个在承诺节点内。这个新需求如果要插队，需要明确挤掉谁——您跟需求方对齐一下优先级，我按结论执行。', brave: '直接说结论：现有排期下这个需求做不了，除非砍掉队列里的某一个。我不做口头承诺然后延期——宁可现在难看一分钟，不要上线前难看一星期。您定砍谁，我马上排。' } }
        ],
        指数A: 10, 指数B: 20
      }
    }
